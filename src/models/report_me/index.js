...
function* query({select, put, call}){
  const { YM: ym, kpi_type } = yield select(({ report_index_me } )=> report_index_me);
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) {
      return history.listen((location) => {
        ...
        const match = pathToRegexp('/report_me/(eventSummaryDetail|eventSupportDetail)?').exec(pathname);
        if (match) {
