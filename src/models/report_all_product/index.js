...
function* query({ select, put, call }) {
  const ym = yield select(({ report_all_product_index }) => report_all_product_index.YM);
...
export default {
  namespace: 'report_all_product_index',
  state: {
    YM: currentDate,
    result: {},
    activeTabKey: '1',
  ...
  effects: {
    ...
    *download({ payload }, { select }) {
      const ym = yield select(({ report_index }) => report_index.YM);
    ...
    setup({ dispatch, history }) {
      return history.listen((location) => {
        ...
        const match = pathToRegexp(
          '/report_all_product/(workingDetail|callRateDetail|customerNumDetail|eventDetail|coachDetail)?',
        ).exec(pathname);
        if (match) {
