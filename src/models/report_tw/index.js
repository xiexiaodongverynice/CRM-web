...
function* query({select, put, call}){
  const { YM: ym, kpi_type } = yield select(({ report_index_tw } )=> report_index_tw);
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) {
      return history.listen((location) => {
        ...
        const match = pathToRegexp('/report_tw/(workingDetail|doctorDetail|doctorCallDetail|doctorCallRateDetail|doctorCallCoverDetail|validDoctorCallCoverDetail)?').exec(pathname);
        if (match) {
