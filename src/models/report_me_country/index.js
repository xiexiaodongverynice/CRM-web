...
function* query({select, put, call}){
  ...
  const { YM: ym, kpi_type, kpiUserId } = yield select(({ report_index_me_country } )=> report_index_me_country);
...
export default {
  namespace: 'report_index_me_country',
  state: {
    YM: currentDate,
    kpi_type: 'me_country',
    result: {},
  ...
  effects: {
    ...
    *download ({ payload }, { select }) {
      const { YM: ym, kpi_type, kpiUserId, kpiUserLevel } = yield select(({ report_index_me_country } )=> report_index_me_country);
      reportService.download({
        kpi_user_level: kpiUserLevel,
        ym,
        kpi_type,
        userId: kpiUserId,
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) {
      return history.listen((location) => {
        ...
        const match = pathToRegexp('/report_me_country').exec(pathname);
        if (match) {
