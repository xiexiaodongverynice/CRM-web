...
const isTimeRange = (beginTime, endTime, compareTime) => {
  ...
  if (n.getTime() - b.getTime() > 0 && n.getTime() - e.getTime() < 0) {
    ...
    consoleUtil.error(
      `当前时间是：${n.getHours()}:${n.getMinutes()}，不在该时间范围内！`,
    );
    return false;
  }
};
...
function fc_combineTime(time, defaultDate) {
  ...
  convertTime = convertTime.replace(/\-/g, ' ').replace(/:/g, ' ').split(' ');
  const d = moment(
    new Date(
      _.get(convertTime, '[0]'),
      _.get(convertTime, '[1]') - 1,
      _.get(convertTime, '[2]'),
      _.get(convertTime, '[3]', 0),
      _.get(convertTime, '[4]', 0),
      _.get(convertTime, '[5]', 0)
    )
  ).valueOf();
  return d
}
