...
const ReportIndexHKSenior = ({dispatch, YM, level, levels, productLines, product_line}) => {
  ...
  const levelChange = (v) => {
    dispatch({
      type: 'report_index_hk_senior/updateState',
      payload: {
        level: v,
      },
    });
  ...
  const download = () => {
    dispatch({
      type: 'report_index_hk_senior/download',
    });
  };
  ...
  return (
    <div className="k_container bg_white">
      <Row className={rowClass}>
        ...
        <Col span={3}>
          <DateMonthDropDown startDate="2018-03" defaultValue={YM} dispatch={dispatch} ns="report_index_hk_senior/updateState"/>
        </Col>
        <Col span={3} style={labelStyle}>
          {
...
const mapStateToProps = (state) => {
  const { YM,  level, levels, productLines, product_line} = state.report_index_hk_senior;
  return {
    YM,
    level,
    levels,
