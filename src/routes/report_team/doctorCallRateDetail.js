...
import { handleResult, mapStateToProps, getDatas, stages, rowClassNameForSM } from './helpers';
...
const craftREPElements = (datas) => {
  const elements = [<UserDetail data={datas[0]} />];
  ...
  elements.push([
    ...
    >
      <span className={styles['text-item']}>
        <strong>{crmIntlUtil.fmtStr(`text.report.target_doctor`)}</strong>
      </span>
    </Row>,
    <Table dataSource={datas} pagination={false}>
      {CommonTargetColumns}
  ...
  elements.push([
    ...
    >
      <span className={styles['text-item']}>
        <strong>{crmIntlUtil.fmtStr(`text.report.non_target_doctor`)}</strong>
      </span>
    </Row>,
    <Table dataSource={datas} pagination={false}>
...
const craftSMElements = (datas, is_SM) => {
  return (
    <Table
      ...
      pagination={false}
      scroll={{ x: 3000 }}
      rowClassName={rowClassNameForSM(is_SM)}
    >
      {ExtraColumnsForSM}
      ...
      <ColumnGroup title={crmIntlUtil.fmtStr('text.report.target_doctor')}>
        {CommonTargetColumns}
      </ColumnGroup>

      <ColumnGroup title={crmIntlUtil.fmtStr('text.report.non_target_doctor')}>
        {CommonNonTargetColumns}
      </ColumnGroup>
    </Table>
  );
