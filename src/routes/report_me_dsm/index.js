...
import { getSubDatas, getSubWhatDatas } from '../report/helpers';
...
import { getRecordByData } from '../report_me_country/helpers';
import { craftSummaryTable } from '../report_me_country/commonElements';

const Column = Table.Column;
const ColumnGroup = Table.ColumnGroup;

const ReportMeDSMIndex = ({ dispatch, location, loading, result = {}, YM, subordinates, kpiUserId, kpiUserLevel}) => {
  ...
  return (
    <div className="k_container bg_white">
      ...
      <Row className={cx(sizeStyles['height-50'], sizeStyles['lineHeight-50'])}>
        <DateMonthDropDown startDate="2017-12" defaultValue={YM} dispatch={dispatch} ns="report_index_me_dsm/updateDate"/>
        {
          craftSubordinatesSeletor(kpiUserId, subordinates, dispatch, 'report_index_me_dsm/updateDate')
        }
        <div className={utilitiesStyles.right}>
          <Button type="primary" onClick={()=>{
            dispatch({
              type: 'report_index_me_dsm/download',
              payload: {
                type: 'summary',
              },
            });
      ...
      {
        craftSupportTable(getSubDatas(data, getSupportRecordData), loading)
      }
    </div>
  );
};
...
const mapStateToProps = (state) => {
  const { result, YM, subordinates, kpiUserId, kpiUserLevel, loading } = state.report_index_me_dsm;
  return {
    result,
    YM,
    subordinates,
...
export default connect(mapStateToProps)(ReportMeDSMIndex);
