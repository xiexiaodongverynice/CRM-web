...
export class DateMonthDropDown extends Component{
  ...
  render(){
    ...
    return (
      <Select defaultValue={defaultValue || _.last(this.months)} style={{ width: 120 }} onChange={(value) => {
        dispatch({
          type: ns,
          payload: {
            YM: value,
      ...
      }}>
        {
          this.months.map(item => {
            ...
            arr.splice(4, 0, '-');
            return (
              <Option value={item} key={item}>{moment(arr.join('')).format('YYYY-MM')}</Option>
            );
          })
        }
      </Select>
...
export const craftSubordinatesSeletor = (kpiUserId, subordinates, dispatch, ns) => {
  ...
  const createOptions= (item, level = 0) => {
    options.push((
      <Select.Option value={`${item.key}`} name={item.label} title={item.label} key={Math.random()} data-level={level} style={getStyle(level)}>
        {
  ...
  return (
    <Select multiple={false} style={{marginLeft: 10, width: 120}} optionLabelProp='name' defaultValue={`${kpiUserId}`} value={`${kpiUserId}`} onSelect={(value, options) => {
      dispatch({
        type: ns,
        payload: {
