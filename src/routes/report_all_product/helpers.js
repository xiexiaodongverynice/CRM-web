...
export const handleResult = (result = {}) => {
  const data = _.chain(result)
    .result('result', {})
    .value();
  const { Level, is_DSM, is_RSM, is_REP, is_SM } = handleRecord(data);
  return {
    data,
    Level,
    is_DSM,
...
export const handleRecord = (data) => {
  const { Level } = data;
  const is_DSM = Level === 'DSM';
  const is_RSM = Level === 'RSM';
...
export const mapStateToProps = (state, asd) => {
  const { result, YM, activeTabKey } = state.report_all_product_index;
  return {
    result,
    YM,
    activeTabKey,
